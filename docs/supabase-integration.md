...
StockTrack uses `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY` in its mobile client, with Expo SQLite-backed `localStorage` for session persistence. The publishable key is intentionally client-visible; data protection comes from Row Level Security and minimal grants rather than a service-role key. The client must never include a database password or service-role secret. [1]
